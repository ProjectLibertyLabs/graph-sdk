...
import { PageData, GraphKeyPair, DsnpKeys, ImportBundle, Action, ConnectAction, Connection, AddGraphKeyAction, KeyData, GraphKeyType, PersistPageUpdate, Update} from './models';
import { Config, ConnectionType, DsnpVersion, PrivacyType } from './models/config';
...
test('getGraphConfig with Mainnet environment should return the graph config', async () => {
    ...
    const schema_id = graph.getSchemaIdFromConfig(environment, ConnectionType.Follow, PrivacyType.Public);
...
test('applyActions with options should honor options', async () => {
    ...
    const imports = exports.filter((update) => update.type === 'PersistPage').map(update => {
        const persist = update as PersistPageUpdate;
        const builder = new ImportBundleBuilder().withDsnpUserId(persist.ownerDsnpUserId).withSchemaId(persist.schemaId).withPageData(persist.pageId, persist.payload, 1000);
...
test('Create and export a new graph', async () => {
    const public_follow_graph_schema_id = graph.getSchemaIdFromConfig(environment, ConnectionType.Follow, PrivacyType.Public);
...
test('Test exportGraph all-user and single-user variants', async () => {
    const publicFollowGraphSchemaId = graph.getSchemaIdFromConfig(environment, ConnectionType.Follow, PrivacyType.Public);
